...
import { Wallet, TrendingUp, Shield, AlertTriangle, ArrowDownCircle, ArrowUpCircle } from "lucide-react";
...
export function PortfolioMargin() {
  ...
  const totalEquity = wallet.connected ? wallet.accountEquity : summary.totalEquity;
  ...
  const handleDeposit = async () => {
    ...
    if (DEMO_MODE) {
      ...
      setTxMessage(`✓ Deposited ${depositAmount} IUSD — tx: ${hash.slice(0, 18)}…`);
      setDepositAmount("");
    }
    setTxLoading(false);
  };
  ...
  const handleWithdraw = async () => {
    ...
    if (DEMO_MODE) {
      await new Promise((r) => setTimeout(r, 1400));
      const hash = "0x" + Math.random().toString(16).slice(2).padEnd(64, "0");
      setTxMessage(`✓ Withdrew ${withdrawAmount} IUSD — tx: ${hash.slice(0, 18)}…`);
      setWithdrawAmount("");
    }
    setTxLoading(false);
  };
    ...
    <div className="h-full overflow-y-auto bg-background p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="rounded-lg border border-border bg-card p-6">
          <h3 className="mb-4 font-semibold" style={{ fontSize: "1.125rem" }}>Collateral Management</h3>
          ...
          {!wallet.connected && (
            <div className="mb-4 flex items-center justify-between rounded-lg bg-ultramarine-50 dark:bg-ultramarine-900/20 border border-ultramarine-200 dark:border-ultramarine-800 px-4 py-3">
              <p className="text-sm text-ultramarine-800 dark:text-ultramarine-200">Connect your wallet to deposit or withdraw collateral.</p>
          ...
          <div className="mb-4 flex gap-1 rounded-lg bg-accent p-1 w-48">
            {(["deposit", "withdraw"] as const).map((t) => (
              <button key={t} onClick={() => { setTab(t); setTxMessage(null); }}
                className={cn("flex-1 rounded px-3 py-1.5 text-sm capitalize transition-colors",
                  tab === t ? "bg-primary text-primary-foreground" : "hover:bg-background")}>
                {t}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <label className="block text-sm text-muted-foreground">
                {tab === "deposit" ? "Deposit Amount (IUSD)" : "Withdraw Amount (IUSD)"}
              </label>
              ...
              <div className="flex gap-2">
                {["100", "500", "1000", "5000"].map((amt) => (
                  <button key={amt} onClick={() => tab === "deposit" ? setDepositAmount(amt) : setWithdrawAmount(amt)}
              ...
              >
                {txLoading ? "Processing…" : tab === "deposit"
                  ? <><ArrowDownCircle className="mr-2 h-4 w-4" />Deposit IUSD</>
                  : <><ArrowUpCircle className="mr-2 h-4 w-4" />Withdraw IUSD</>}
              </Button>
            ...
            <div className="space-y-3 rounded-lg bg-muted p-4 text-sm">
              ...
              {[
                ...
                ["Deposited",        formatUSD(summary.totalEquity)],
                ["Available Margin", formatUSD(summary.availableBalance)],
                ["Used Margin",      formatUSD(summary.initialMargin)],
                ["Health Ratio",     `${((summary.totalEquity / Math.max(summary.maintenanceMargin, 1)) * 100).toFixed(0)}%`],
              ].map(([label, value]) => (
            ...
            <table className="w-full text-sm">
              ...
              <tfoot className="border-t-2 border-border">
                <tr>
                  ...
                  <td />
                  <td className="px-4 py-4 text-right font-mono font-semibold">{formatUSD(assets.reduce((s,a) => s+a.value,0))}</td>
